...
class Custom_Fields {
  ...
  private function render_all_code_snippet( $fields ) {
    ...
    foreach ( $fields as $field ) {
      ...
      $var_name = $this->code_var_name( $key );
      $meta_key_literal = var_export( $meta_key, true );

      $code .= "// --- {$label} ({$type}) ---\n";

      if ( $type === 'repeater' ) {
        $sub_fields = $field['sub_fields'] ?? [];
        $code .= "{$var_name}_rows = get_post_meta( get_the_ID(), {$meta_key_literal}, true );\n";
        ...
        foreach ( $sub_fields as $sub ) {
          $sub_key = $sub['key'];
          $sub_lbl = $sub['label'] ?? '';
          $sub_key_literal = var_export( $sub_key, true );

          if ( isset($sub['type']) && $sub['type'] === 'image' ) {
             $code .= "    // {$sub_lbl} (画像ID)\n";
             $code .= "    \$img_id = \$row[{$sub_key_literal}] ?? 0;\n";
             $code .= "    echo wp_get_attachment_image( \$img_id, 'medium' );\n";
          } elseif ( isset($sub['type']) && $sub['type'] === 'checkbox' ) {
             $code .= "    // {$sub_lbl} (配列)\n";
             $code .= "    \$sub_vals = \$row[{$sub_key_literal}] ?? [];\n";
             $code .= "    echo esc_html( implode(', ', \$sub_vals) );\n";
          } elseif ( isset($sub['type']) && $sub['type'] === 'post' ) {
             $code .= "    // {$sub_lbl} (記事ID)\n";
             $code .= "    \$pid = \$row[{$sub_key_literal}] ?? 0;\n";
             $code .= "    if ( \$pid ) echo '<a href=\"' . esc_url( get_permalink( \$pid ) ) . '\">' . esc_html( get_the_title( \$pid ) ) . '</a>';\n";
             ...
             $code .= "    // {$sub_lbl}\n";
             $code .= "    echo esc_html( \$row[{$sub_key_literal}] ?? '' );\n";
          }
          $code .= "    echo '<br>';\n";
        }
        $code .= "  }\n";
        ...
        $code .= "{$var_name}_id = get_post_meta( get_the_ID(), {$meta_key_literal}, true );\n";
        ...
        $code .= "{$var_name}_id = get_post_meta( get_the_ID(), {$meta_key_literal}, true );\n";
        $code .= "if ( {$var_name}_id ) {\n";
        $code .= "  echo '<a href=\"' . esc_url( get_permalink( {$var_name}_id ) ) . '\">' . esc_html( get_the_title( {$var_name}_id ) ) . '</a>';\n";
        $code .= "}\n";
  ...
  /**
   * 出力コードで使うPHP変数名を安全な形にする
   */
  private function code_var_name( $key ) {
    $name = preg_replace( '/[^a-zA-Z0-9_]/', '_', (string) $key );
  ...
  private function sanitize_field_value( $field, $value ) {
    ...
    if ( is_array( $value ) ) {
      // 配列入力を許可するのはcheckboxのみ。スカラー型への配列POSTは保存しない。
      if ( 'checkbox' !== $type ) {
        return '';
      }
    ...
    switch ( $type ) {
      case 'image':
        $attachment_id = $value ? absint( $value ) : 0;
        if ( ! $attachment_id ) {
          return '';
        }
        $attachment = get_post( $attachment_id );
        return ( $attachment && 'attachment' === $attachment->post_type && wp_attachment_is_image( $attachment_id ) ) ? $attachment_id : '';
      case 'post':
